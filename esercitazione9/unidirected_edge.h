...
public:
    unidirected_edge(const T& node1, const T& node2) {
        ...
        node_end = std::max(node1, node2);
    }

    T from() const { return node_from; }
    T to() const { return node_end; }

    bool operator<(const unidirected_edge<T>& other) const {     // verifica se l'arco è minore secondo questo criterio di ordinamento
        if (node_from != other.node_from) {
            return node_from < other.node_from;
        }
        return node_end < other.node_end;
    }

    bool operator==(const unidirected_edge<T>& other) const {       // controlla se i due archi sono uguali
        return node_from == other.node_from && node_end == other.node_end;
    }
};
